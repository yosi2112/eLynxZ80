...
Console colour output is disabled when `-OutputPath` is specified.

Use `-NoColor` to disable colour output only.
...
- `-NoBackup` reduces recovery options. Keep the default backup behaviour unless the target files can be regenerated reliably.
