...
|---|---|---|---|
| `buildall.ps1` | Top-level build entry point for ROMs, the CP/M disk, and CP/M utilities. It checks the required archives in `tool\lynxZ80\build\arch` and verifies that `asw.exe` and `p2bin.exe` are available on `PATH`. | `tool\lynxZ80\build\arch\cpm2-asm.zip`, `tool\lynxZ80\build\arch\cpm22-b.zip`, individual build scripts | ROM/IMG/COM artifacts under `src\vm\Lynxz80\build` |
...
- `KH-Dot-Dougenzaka-16.ttf` is licensed separately under the SIL Open Font License 1.1. Generated font ROM data derived from that font should be treated under the same license when redistributed.
